 ...
 function sing($sn){
	...
	//A better fix would be removing $hplsinfo from $mapinfo['plsinfo'] after processing the song. But still.
	...
	if ($sn=="Alicemagic"){
		...
		addnews($now,'song',$name,$mapinfo['plsinfo'][$pls],$noiseinfo[$sn],$nick);
		return;
...
//	addnoise($sn,'__',$now,$pls,0,0,$sn);
//	addnews($now,'song',$name,$mapinfo['plsinfo'][$pls],$noiseinfo[$sn]);
	return;
 }
 ?>