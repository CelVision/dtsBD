...
function itemuse($itmn,&$data=NULL) {
	...
	if(strpos ( $itmk, 'W' ) === 0 || strpos ( $itmk, 'D' ) === 0 || strpos ( $itmk, 'A' ) === 0 || strpos ( $itmk, 'ss' ) === 0){
		...
		if ($mefct > 0 && $mefct !== 999) {
			$log .= "身体里有种力量涌出来！<br>你的{$mdname}提高了<span class=\"yellow\">$mefct</span>点！<br>";
		} elseif ($mefct == 999) {
			$log .= "你一口吃下手中的药物，感到元素之力涌了上来！获得<span class=\"yellow\">$skillpointgain</span>点技能点！<br>";
		} elseif ($mefct == 0) {
		...
		if ($itme > 0) {
			...
			if($club == 7 && $itm !== '七彩水晶球'){
				...
				$itme--;
				$log .= "消耗了<span class=\"yellow\">$itm</span>的元素能量。<br>";
			}else{
				$itme--;
				$log .= "消耗了<span class=\"yellow\">$itm</span>的电力。<br>";
			}
...
/* 		if ($itms != $nosta) {
	...
	# Special check for a poisoned fireseed item, WIP for now.	
		...
		if ($itm == '电池') {
			...
			//global $mapinfo['plsinfo'],$nosta,$db,$tablepre;
			...
			if(!empty($ndata)){
				foreach($ndata as $key => &$val){
					$npls = $val['pls'];
					while($npls == $val['pls']){
						$npls = rand(1,count($mapinfo['plsinfo'])-1);
					}				
					$val['pls'] = $npls;$npls = $mapinfo['plsinfo'][$npls];
...
//			}
		...
		//文案
			...
			//global $mapinfo['plsinfo'],$db,$tablepre;
			if($itm =='→【单兵撤退按钮】←'){
				...
				$kitm2="［ＤＩＬＩＧＥＮＣＥ］";
				$kitm3="［ＣＯＮＳＣＩＥＮＣＥ］";
				$rndpls1= rand(1,count($mapinfo['plsinfo'])-2);
				$rndpls2= rand(1,count($mapinfo['plsinfo'])-2);
				$rndpls3= rand(1,count($mapinfo['plsinfo'])-2);
				...
				$plsname1 = $mapinfo['plsinfo'][$rndpls1];
				$plsname2 = $mapinfo['plsinfo'][$rndpls2];
				$plsname3 = $mapinfo['plsinfo'][$rndpls3];
			...
			elseif ($itme == 20){ // 元素大师特殊处理
				...
				【内测玩法·元素大师使用规则】<br>
				<br>
				【其之一】这世■的一切都由六种■素组成。<br>
				【其之二】每种■素都能组成一种武器或防■。<br>
				【其之三】当你捡到物品后，便可将其提炼成■素。<br>
				【其之四】此外，看起来没有用的尸■也可被提炼，不过后果自负。<br>
				【其之五】提炼时偶尔会蹦出■殊信■，最好将它们记录下来。<br>
				【其之六】提炼出的■素，可以通过「■素合成」产出各种物品。<br>
				【其之七】相对是这个世界的摄理之一，如果过于追求■字，就无法体现■殊性。<br>
			...
			$log .= '你读着纸条上的内容：<br>“执■官其实都是幻影，那个红■的身上应该有召唤幻影的玩意。”<br>“用那个东西然后打■幻影的话能用■戏解除■匙出去吧。”<br>';
		} elseif ($itm == '提示纸条B') {
			$log .= '纸条上模糊的写着：<br>“是你...是■...■■...”<br>纸条已经损坏到完全没法读了<br>';
		} elseif ($itm == '提示纸条C') {
			$log .= '你读着纸条上的内容：<br>“如果能■前找到那个■洞...”<br>“也■就能打开意■不到■门……”<br>';
		} elseif ($itm == '提示纸条D') {
			$log .= '你读着纸条上的内容：<br>“Digital.”<br>“合成幻■控制密■，你需要三个■■....”<br>';
		} elseif ($itm == '提示纸条E') {
			...
			$log .= '纸条摸起来有些冰凉：<br>“■人吗？”<br>“如果你看■这个纸条，请把它■到世界的尽■！”<br>';
		} elseif ($itm == '提示纸条L') {
			...
			$log .= '你读着纸条上的内容：<br>“Surface.”<br>“想要■解管■后台，你需要输入更高级的■■”';
		} elseif ($itm == '提示纸条T') {
			$log .= '你读着纸条上的内容：<br>“Tactic.”<br>“#$%@.....?”<br>这段提示似乎损坏了<br>';
		} elseif ($itm == '提示纸条U') {
			$log .= '你读着纸条上的内容：<br>“纸条啥的……”<br>“希望你不会在纸条上写自己的遗书。”<br>“那会很无聊的。”<br>';
		} elseif ($itm == '空白纸条') {
			$log .= '<br>空白指令为'.$gamevars['rand_controls'][0][0].'。<br>填入更高级的代码残片以使用<br>';
		} elseif ($itm == '赤红钥环') {
			$log .= '<br>你捏住了赤红色的钥匙环，突然感受到手上传来一阵吃痛！<br>'.$gamevars['rand_controls'][1][0].'<br>手掌上被钥匙灼伤出一串字符！<br>';
			$inf = str_replace('u','',$inf);
			$inf = ($inf . 'u');
		} elseif ($itm == '冰蓝钥柄') {
			$log .= '<br>'.$gamevars['rand_controls'][2][0].'。';
		} elseif ($itm == '人品探测器') {
			//global $rp;
			...
			if ($selfdestructdice1 > 0){
				...
				$rndhappypls= rand(1,count($mapinfo['plsinfo'])-2);
