<?php
	...
	{
		global $log,$cskills,$now,$club_skillslist,$weather,$gamevars,$wthinfo,$db,$tablepre,$exp;
		global $elements_info,$blacklotuscost;
		...
		if($event == 'crystal')
		{
			...
			if($rp < 1000){
				...
				$slde = round(1000 * get_skillvars($sk,'sldr') / 100);
			}
		...
		if($event == 'windfall')
		{
			if($exp >= 100){
			    ...
			    $numbers[5] += $t;
			    $log .= "你向元素口袋中注入了元素能量……然后看着元素在你的口袋中翻滚……搅拌……<br>";
