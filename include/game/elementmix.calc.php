<?php
	...
	{
		global $log,$elements_info,$gamecfg,$gamevars,$typeinfo,$exp;
		include config('elementmix',$gamecfg);
		if(!isset($data))
		{
			global $pdata;
	...
	{
		global $log,$mode,$typeinfo,$elements_info,$gamecfg,$gamevars,$exp;
		include config('elementmix',$gamecfg);
		if(!isset($data))
		{
			global $pdata;
	...
	function emix_calc_maxenum(&$data=NULL)
	{
		...
		extract($data,EXTR_REFS);
		//这里是胶冻，元素大师调整过后变成了设限指数增长的形式，即增长速度为dy/dx = ky(A-y)的形式，A是上限，k是增长系数，调整这个式子的话可以先填参数再把这个式子积分出来。
		//这里在经验无穷大的时候趋近300000，在经验趋近0的时候趋近400.
		$max_enum = round(300000/(1 + 749 * exp(-300000*$exp*0.00000000612)));
		return $max_enum;
	}
	...
	{
		global $log,$elements_info,$gamecfg,$exp;
		include config('elementmix',$gamecfg);
		if(!isset($data))
		{
			global $pdata;
	...
	{
		global $log,$elements_info,$gamecfg,$gamevars,$exp;
		include config('elementmix',$gamecfg);
		if(!isset($data))
		{
			global $pdata;
		...
		$max_sk = max(1,min(7,ceil(sqrt(sqrt($exp)))));
